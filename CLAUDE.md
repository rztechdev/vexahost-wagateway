...
**Harga di halaman depan masih etalase.** Tiga paket (150rb/300rb/550rb per bulan, tahunan = ×10) dipasang di `resources/views/welcome.blade.php` untuk mempercantik halaman, atas permintaan Ryan (17 Agu 2026). Di belakangnya belum ada apa-apa: tidak ada langganan, tidak ada penagihan, dan batas workspace maupun kuota per paket **tidak ditegakkan** di aplikasi — `monthly_message_quota` diisi per workspace, bukan per paket. Tombolnya sengaja mengarah ke pendaftaran biasa. Sebelum angka itu benar-benar ditagihkan, batasnya harus ditegakkan lebih dulu.

**Warna diambil dari satu tempat.** Palet tema "Nature" (hijau, mengikuti logo) hidup sebagai variabel CSS di `resources/css/app.css`; `resources/css/auth.css` punya salinannya sendiri karena halaman auth memakai latar WebGL dan efek kaca yang butuh warna solid. Menambah warna baru berarti menambah token, bukan menulis `bg-green-600` di Blade. Yang sengaja dikecualikan: status sesi (amber/biru/merah — lima keadaan tidak mungkin dibedakan dengan lima rona hijau) dan blok kode (selalu gelap di kedua mode). Font **tidak** ikut preset tema — Anthropic Serif di aplikasi, Outfit + Plus Jakarta Sans di halaman auth.

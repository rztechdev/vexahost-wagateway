...
@section('content')
    <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
            ...
            <p class="mt-0.5 text-sm text-muted-foreground">
                Tiket #{{ $ticket->id }} ·
                <a href="{{ route('admin.workspaces.show', $ticket->workspace_id) }}" class="hover:underline">{{ $ticket->workspace?->name }}</a>
                · {{ $ticket->labelKategori() }} · prioritas {{ strtolower($ticket->labelPrioritas()) }}
                · dibuka {{ $ticket->user?->email ?? 'akun terhapus' }}
            </p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            ...
            <form method="POST" action="{{ route('admin.tickets.status', $ticket->id) }}">
                ...
                <button class="rounded-lg border border-border px-3 py-1.5 text-xs transition hover:bg-muted">
                    {{ $ticket->isClosed() ? 'Buka lagi' : 'Tutup tanpa membalas' }}
                </button>
            </form>
        </div>
    </div>
                ...
                <div @class(['rounded-lg p-4', 'bg-primary/5 border border-primary/15' => $pesan->is_from_admin, 'bg-muted/40' => ! $pesan->is_from_admin])>
                    <div class="mb-2 flex flex-wrap items-baseline justify-between gap-2 text-xs">
                        <span class="font-medium">
                            {{ $pesan->is_from_admin ? 'Tim VexaHost' : ($pesan->user?->name ?? 'Pelanggan') }}
                        </span>
                    ...
                    @if ($pesan->punyaLampiran())
                        <a href="{{ route('admin.tickets.attachment', [$ticket->id, $pesan->id]) }}"
    ...
    <x-card title="Balas"
            subtitle="Pelanggan dikabari lewat WhatsApp dan email. Isi balasannya sendiri tidak ikut dikirim — tiket bisa memuat kunci API yang mereka tempelkan sendiri."
            class="mt-5">
        <form method="POST" action="{{ route('admin.tickets.reply', $ticket->id) }}" enctype="multipart/form-data" class="space-y-3" data-validasi>
            @csrf
            <textarea name="body" required rows="5" maxlength="5000"
            ...
            <label class="flex items-center gap-2 text-sm">
                <input type="checkbox" name="tutup" value="1" class="rounded border-border">
                Tutup tiket setelah balasan ini
            </label>

...
<div x-data="{ sidebar: false }" class="min-h-full">
    ...
    <header class="sticky top-3 z-30 px-3 lg:top-4 lg:px-6">
        <div class="mx-auto flex max-w-7xl items-center gap-3 rounded-2xl border border-border bg-background/90 px-4 py-3 backdrop-blur-md shadow-sm">
    ...
    <div class="mx-auto flex max-w-7xl gap-6 px-4 py-6">
        <aside
            class="fixed top-3 bottom-3 left-3 z-40 w-64 shrink-0 overflow-y-auto rounded-2xl border border-border bg-background p-5 shadow-2xl transition-transform duration-300 ease-in-out lg:static lg:bottom-auto lg:left-auto lg:top-auto lg:z-0 lg:block lg:w-60 lg:translate-x-0 lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none"
            ...
            <div class="mt-6 border-t border-border pt-4 lg:hidden">
                <a href="{{ route('docs.index') }}" class="block rounded-lg px-3 py-2 text-muted-foreground hover:bg-muted hover:text-foreground">
                    Dokumentasi
                </a>
                <button @click="document.documentElement.classList.toggle('dark'); localStorage.theme = document.documentElement.classList.contains('dark') ? 'dark' : 'light'" 
                ...
                <form method="POST" action="{{ route('logout') }}" class="mt-1">
                    @csrf
                    <button class="flex w-full rounded-lg px-3 py-2 text-muted-foreground hover:bg-muted hover:text-foreground text-left">Keluar</button>
                </form>
            </div>
        </aside>
